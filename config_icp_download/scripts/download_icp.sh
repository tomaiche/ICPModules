...
if [ -n $PARAM_ICP ]; then
  ...
  if [[ $PARAM_ICP =~ $URL_REGEX ]]; then
    if [[ -e "/root/ibm-cloud-private-x86_64-${PARAM_ICP_VERSION}/${ICP_INSTALLER_FILE_NAME}" ]]; then
      ...
      if [[ "$ARCH" = "x86_64" && "$PARAM_ICP_VERSION" = "3.1.0" ]]; then
          printf "\033[32m[*] Tagging ICP inception image ibmcom/icp-inception-amd64:3.1.0-ee to ibmcom/icp-inception:3.1.0-ee\033[0m\n"
