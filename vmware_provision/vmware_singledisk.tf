resource "vsphere_virtual_machine" "vm" {
  ...
  connection {
    ...
    host     = "${vsphere_virtual_machine.vm.clone.0.customize.0.network_interface.0.ipv4_address}"
...
resource "vsphere_virtual_machine" "vm2disk" {
  ...
  connection {
    ...
    user     = "${var.vm_os_user}"
    password = "${var.vm_os_password}"
    host     = "${vsphere_virtual_machine.vm2disk.clone.0.customize.0.network_interface.0.ipv4_address}"
    bastion_host        = "${var.bastion_host}"
    bastion_user        = "${var.bastion_user}"
    bastion_private_key = "${ length(var.bastion_private_key) > 0 ? base64decode(var.bastion_private_key) : var.bastion_private_key}"
    bastion_port        = "${var.bastion_port}"
